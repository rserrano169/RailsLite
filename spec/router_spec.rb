...
describe Route do
  ...
  describe "#run" do
    before(:all) { class TestController; end }
    after(:all) { Object.send(:remove_const, "TestController") }

    it "instantiates controller and invokes action" do
      allow(req).to receive(:path) { "/users" }

      test_controller_class = TestController
      test_controller_instance = TestController.new
      allow(test_controller_instance).to receive(:invoke_action)
      allow(test_controller_class).to receive(:new).with(req, res, {}) do
        test_controller_instance
      end
      expect(test_controller_instance).to receive(:invoke_action)
      index_route = Route.new(Regexp.new("^/users$"), :get, test_controller_class, :index)
